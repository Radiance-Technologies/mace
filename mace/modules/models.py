...
class MACE(torch.nn.Module):
    ...
    def __init__(self,
        ...
        interaction_irreps = (sh_irreps_inter *
                              num_features).sort()[0].simplify()
        interaction_irreps_first = (sh_irreps *
                                    num_features).sort()[0].simplify()
        ...
        for i in range(num_interactions - 1):
            if i == num_interactions - 2 and not keep_last_layer_irreps:
                hidden_irreps_out = str(
                    hidden_irreps[0])  # Select only scalars for last layer
            else:
                hidden_irreps_out = hidden_irreps
            ...
            if i == num_interactions - 2:
                if compute_uncertainty:
                    self.readouts.append(
                        readout_cls(
                            hidden_irreps_out,
                            (2 * len(heads) * MLP_irreps).simplify(),
                            gate,
                            o3.Irreps(f"{2 * len(heads)}x0e"),
                            len(heads) * 2,
                            cueq_config,
    ...
    def forward(
        ...
        e0 = scatter_sum(src=node_e0,
                         ...
                         dim_size=num_graphs).to(
                             vectors.dtype)  # [n_graphs, n_heads]
        # Embeddings
        ...
        if hasattr(self, "pair_repulsion"):
            ...
            pair_energy = scatter_sum(src=pair_node_energy,
                                      index=data["batch"],
                                      dim=-1,
                                      dim_size=num_graphs)  # [n_graphs,]
        else:
        ...
        node_energy = torch.sum(torch.stack(node_energies_list, dim=-1),
                                dim=-1)
        node_feats_out = torch.cat(node_feats_concat, dim=-1)

        if self.compute_uncertainty and len(var_logits_list) > 0:
            node_var_logits = torch.sum(torch.stack(var_logits_list, dim=-1),
                                        dim=-1)
            node_var_logits_head = node_var_logits[
                torch.arange(node_var_logits.size(0)), node_heads]
            energy_var_logits = scatter_sum(node_var_logits_head,
                                            data["batch"],
                                            dim=0,
                                            dim_size=num_graphs).squeeze(-1)
        else:
            energy_var_logits = None
        ...
        if self.compute_uncertainty and energy_var_logits is not None:
            f_var, v_var, s_var, _, _ = get_outputs(
                energy=energy_var_logits,
                positions=positions,
                displacement=displacement,
                vectors=vectors,
                cell=cell,
            ...
            if f_var is not None:
                forces_var_logits = torch.abs(f_var)
            if v_var is not None:
                virials_var_logits = torch.abs(v_var)
            if s_var is not None:
                stress_var_logits = torch.abs(s_var)
...
class ScaleShiftMACE(MACE):
    ...
    def forward(
        ...
        if hasattr(self, "joint_embedding"):
            ...
            if hasattr(self, "embedding_readout"):
                embedding_node_energy = torch.atleast_1d(
                    self.embedding_readout(node_feats,
                                           node_heads)[num_atoms_arange,
                                                       node_heads].squeeze(-1))
                embedding_energy = scatter_sum(
                    src=embedding_node_energy,
                    index=data["batch"],
                    dim=0,
        ...
        if self.compute_uncertainty and len(var_logits_list) > 0:
            node_var_logits = torch.sum(torch.stack(var_logits_list, dim=0),
                                        dim=0)
            energy_var_logits = scatter_sum(node_var_logits,
                                            data["batch"],
                                            dim=-1,
                                            dim_size=num_graphs)
        else:
...
class AtomicDipolesMACE(torch.nn.Module):
    ...
    def __init__(
        ...
        for i in range(num_interactions - 1):
            if i == num_interactions - 2:
                assert (
                    len(hidden_irreps)
                    > 1), "To predict dipoles use at least l=1 hidden_irreps"
                hidden_irreps_out = str(
                    hidden_irreps[1])  # Select only l=1 vectors for last layer
            else:
                hidden_irreps_out = hidden_irreps
            inter = interaction_cls(
                node_attrs_irreps=node_attr_irreps,
            ...
            if i == num_interactions - 2:
                self.readouts.append(
                    NonLinearDipoleReadoutBlock(hidden_irreps_out,
                                                MLP_irreps,
                                                gate,
                                                dipole_only=True))
            else:
    ...
    def forward(
        ...
        contributions_dipoles = torch.stack(
            dipoles, dim=-1)  # [n_nodes,3,n_contributions]
        atomic_dipoles = torch.sum(contributions_dipoles,
                                   dim=-1)  # [n_nodes,3]
...
class AtomicDielectricMACE(torch.nn.Module):
    ...
    def __init__(
        ...
        self.register_buffer("polarizability_mean",
                             torch.zeros(3, 3))  # 3x3 matrix flattened
        ...
        if means_stds is not None:
            ...
            if "polarizability_mean" in means_stds:
                self.polarizability_mean.data.copy_(
                    means_stds["polarizability_mean"])
            if "polarizability_std" in means_stds:
                self.polarizability_std.data.copy_(
                    means_stds["polarizability_std"])
    ...
    def forward(
        ...
        bool = False,  # no training on derivatives
            compute_edge_forces: bool = False,  # pylint: disable=W0613
            compute_atomic_stresses: bool = False,  # pylint: disable=W0613
    ) -> Dict[str, Optional[torch.Tensor]]:
        assert compute_force is False
        ...
        total_charge_excess = scatter_mean(
            src=atomic_charges, index=data["batch"],
            dim_size=num_graphs) - (data["total_charge"] / num_atoms)
        ...
        if self.use_polarizability:
            ...
            contributions_polarizabilities = torch.stack(
                polarizabilities, dim=-1)  # [n_nodes,6,n_contributions]
...
class EnergyDipolesMACE(torch.nn.Module):
    ...
    def forward(
        ...
        for interaction, product, readout in zip(self.interactions,
            ...
            energy = scatter_sum(src=node_energies,
                                 index=data["batch"],
                                 dim=-1,
                                 dim_size=num_graphs)  # [n_graphs,]
            energies.append(energy)
        ...
        node_energy = torch.sum(node_energy_contributions,
                                dim=-1)  # [n_nodes, ]
        contributions_dipoles = torch.stack(
            dipoles, dim=-1)  # [n_nodes,3,n_contributions]
        atomic_dipoles = torch.sum(contributions_dipoles,
